  ...
  <svg viewBox="0 0 400 220" preserveAspectRatio="xMidYMid slice">
    <defs>
      <radialGradient id="g1" cx="50%" cy="35%" r="60%">
        <stop offset="0%" stop-color="#1c0505"/><stop offset="100%" stop-color="#050504"/>
      </radialGradient>
      <filter id="rough">
    ...
    <g filter="url(#rough)" opacity="0.9">
      <ellipse cx="200" cy="95" rx="62" ry="78" fill="#2a1010"/>
      <rect x="150" y="150" width="100" height="70" fill="#1a0808"/>
      <line x1="150" y1="70" x2="250" y2="130" stroke="#5a0f0f" stroke-width="3"/>
      <line x1="250" y1="70" x2="150" y2="130" stroke="#5a0f0f" stroke-width="3"/>
    </g>
    <rect width="400" height="220" fill="#000" opacity="0.15"/>
  </svg>`;
}
...
function screenKillerReveal(){
  ...
  render(`
    ...
    You don't have long before it's fully dark. And once I start looking for you — I won't stop."</p>
  ...
  <svg viewBox="0 0 400 220" preserveAspectRatio="xMidYMid slice">
    <defs>
      <radialGradient id="g2" cx="50%" cy="30%" r="65%">
        <stop offset="0%" stop-color="#141607"/><stop offset="100%" stop-color="#050504"/>
      </radialGradient>
      <filter id="rough2">
        <feTurbulence type="fractalNoise" baseFrequency="0.02 0.1" numOctaves="2" seed="3" result="n"/>
        <feDisplacementMap in="SourceGraphic" in2="n" scale="14"/>
      </filter>
    </defs>
    <rect width="400" height="220" fill="url(#g2)"/>
    <g filter="url(#rough2)">
      <rect x="160" y="60" width="80" height="160" fill="#0d0c08"/>
      <ellipse cx="200" cy="55" rx="34" ry="42" fill="#3a1414"/>
      <line x1="200" y1="97" x2="200" y2="130" stroke="#601616" stroke-width="2"/>
    </g>
    <rect width="400" height="220" fill="#000" opacity="0.2"/>
  </svg>`;
}
...
========================================================= */
const MAP_W = 560, MAP_H = 340; // used only for container aspect ratio
...
function initScene(){
  ...
  const kBody = new THREE.Mesh(new THREE.CylinderGeometry(5,5,18,8), new THREE.MeshStandardMaterial({color:0x2a0a0a, emissive:0x7a1414, emissiveIntensity:0.9}));
  kBody.position.y = 9;
  const kHead = new THREE.Mesh(new THREE.SphereGeometry(5,8,8), new THREE.MeshStandardMaterial({color:0x3a1414, emissive:0xb8281f, emissiveIntensity:1.1}));
  kHead.position.y = 21;
  killerMesh.add(kBody); killerMesh.add(kHead);
...
function updateKiller(dt){
  ...
  const baseSpeed = 60 + urgency*70 + Math.min(map.elapsed,60)*0.6;
...
function handleDanger(d, dt){
  ...
  if(visibleNow && !map.killerWasVisible){ flashScreen(); playStinger(); }
